...
public interface ProprietarioRepository extends JpaRepository<Proprietario, Integer> {
    ...
    @Query(value ="SELECT sub.id, sub.nome_estabelecimento, sub.estado, sub.cidade, sub.bairro, sub.avaliacao_media, sub.descricao, sub.endereco_id," +
            "sub.logradouro, sub.telefone, sub.url_icone, sub.usuarios_id, sub.deficiencias_id FROM " +
            ...
            "JOIN  proprietario_deficiencias_table pdt ON pdt.proprietario_id = pt.id) sub WHERE " +
            "(sub.nome_Estabelecimento = :nome OR null is null OR null ='') AND " +
            "(sub.estado = :estado OR null is null OR null = '') AND " +
            "(sub.bairro = :bairro OR null is null OR null = '') AND " +
            "(sub.cidade = :cidade OR null is null OR null = '') AND " +
            "(sub.avaliacao_media >= :avMedia OR null is null OR null = '') ", nativeQuery = true)
    List<Proprietario> findProprieratioByfilter(@Param("nome")  String nomeEstabelecimento,
